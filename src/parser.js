    ...
    case 'object':
      ...
      else
      {
        ...
        switch (element['#name'])
        {
          case 'ref':
            return s
              + markdown.refLink(toMarkdown(element.$$), element.$.refid);
          ...
          case 'title':
            s = '\n#'
              + '#'.repeat(context[context.length - 1]['#name'].slice(-1))
              + ' ' + element._ + '\n';
            break;
...
module.exports = {
  ...
  parseMember : function(member, section, memberdef)
  {
    ...
    switch (member.kind)
    {
      ...
      case 'function':
        ...
        m = m.concat(memberdef.argsstring[0]._.match(/noexcept$/)
          ? ' noexcept' : '');
        m = m.concat(memberdef.argsstring[0]._.match(/=\s*delete$/)
          ? ' = delete' : '');
        m = m.concat(memberdef.argsstring[0]._.match(/=\s*default/)
          ? ' = default' : '');
        break;
  ...
  assignToNamespace : function(compound, child)
  {
    ...
    {
      console.assert('namespace mismatch: ',
                     compound.name, '!=', child.namespace);
    }
  ...
  extractPageSections : function(page, elements)
  {
    elements.forEach(function(element)
    {
      if (element['#name'] == 'sect1'
        || element['#name'] == 'sect2' || element['#name'] == 'sect3')
      {
        const id     = element.$.id;
        const member = {
  ...
  parseCompound : function(compound, compounddef)
  {
    ...
    compound.proto = helpers.inline([ compound.kind, ' ',
                                      markdown.refLink(compound.name,
                                                       compound.refid) ]);
      ...
      case 'group':
        ...
        if (compounddef.innerclass)
        {
          compounddef.innerclass.forEach(function(innerclassdef)
          {
            ...
            {
              // log.verbose('Assign ' + innerclassdef.$.refid
              //    + ' to namespace ' + compound.name);
        ...
        if (compounddef.innernamespace)
        {
          ...
          compounddef.innernamespace.forEach(function(namespacedef)
          {
            if (compound.kind == 'group')
            {
              // log.verbose('Assign namespace ' + namespacedef.$.refid
              //    + ' to group ' + compound.name);
              this.assignNamespaceToGroup(
                compound, this.references[namespacedef.$.refid]);
            }
          }.bind(this));
        }
        break;
  ...
  parseIndex : function(root, index, options)
  {
    ...
    {
      let doxygen; const compound
        = root.find(element.$.refid, element.name[0], true);
      ...
      { // && compound.kind !== 'file'
        ...
        doxygen = fs.readFileSync(path.join(options.directory,
                                            compound.refid + '.xml'), 'utf8');
